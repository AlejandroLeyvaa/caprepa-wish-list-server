...
router.put('/user-update-image', upload.single("user-image"), async(req, res, next) => {
    try {
        const { body, file } = req;
        const { user_id } = body;
        if (file) {
...
router.post('/sign-up', async(req, res, next) => {
    try {
        ...
        if(!body.user_name.length || !body.user_email.length || !body.department_id) {
            res.send({
                valid: false,
...
router.post('/login', async(req, res, next) => {
    try {
        ...
        const correctPassword = await bcrypt.compare(user_password, storedUserPassword);
